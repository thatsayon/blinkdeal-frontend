...
export default function ProductCategoryDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [form, setForm] = useState<ProductCategory>({ name: "", slug: "", icon: "", color: "", bg_color: "" });
  ...
  const loadCategory = async () => {
    try {
      const data = await fetchAdminAPI<ProductCategory>(`/admin/product-categories/${id}/`);
      setForm(data);
    } catch (e) {
      console.error("Failed to load product category");
      showMessage("error", "Failed to load product category data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!isNew) {
        await fetchAdminAPI(`/admin/product-categories/${id}/`, {
          method: "PUT",
          body: JSON.stringify(form),
        });
      } else {
        await fetchAdminAPI("/admin/product-categories/", {
          method: "POST",
          body: JSON.stringify(form),
        });
      }
      ...
      showMessage("error", "Error saving product category. Please try again.");
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link href="/admin/product-categories" className="p-2 text-gray-500 hover:text-gray-900 bg-white rounded-lg border border-gray-200 shadow-sm transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            {!isNew ? <Edit2 className="w-6 h-6 text-indigo-500" /> : <Plus className="w-6 h-6 text-indigo-500" />}
            {!isNew ? "Edit Product Category" : "Add New Product Category"}
          </h1>
        </div>
      </div>
